...
def dump_sample(data, offset, smpdir, smp_num):
    ...
    if smp_name:
        ...
        with open(join(smpdir, smp_name + '.sgz'), 'wb') as sgz:
            sgz.write(compress(data[smp_point:smp_point+smp_length]))
...
def module_to_text(inpath, outpath, smpdir):
    ...
    with open(outpath, 'w') as f:
        f.write('[Header]\n')
        f.write(encode(data[:0xc0]).decode() + '\n')
        f.write('[Orders]\n')
        f.write(encode(data[0xc0:0xc0+ordnum]).decode() + '\n')
        for i, offset in enumerate(ins_offsets):
            f.write('[Instrument%02d]\n' % (i+1))
            f.write(encode(data[offset:offset+554]).decode() + '\n')
        for i, offset in enumerate(smp_offsets):
            f.write('[Sample%02d]\n' % (i+1))
            dump_sample(data, offset, smpdir, i+1)
            f.write(encode(data[offset:offset+0x50]).decode() + '\n')
        for i, offset in enumerate(pat_offsets):
            f.write('[Pattern%03d]\n' % i)
            length = unpack_from('=H', data, offset)[0]
            f.write(encode(data[offset:offset+8+length]).decode() + '\n')
...
def text_to_module(inpath, outpath, smpdir):
    ...
    while i < len(lines):
        if lines[i].startswith('[Pattern'):
            ...
            length = unpack_from('H', decode(lines[i+1]), 0)[0]
            cur_offset += 8 + length
        i += 1
            ...
            if any(line.startswith('[' + s) for s in sections):
                ...
                if is_sample:
                    ...
                    if smp_name and smp_flags & FLAG_SAMPLE_ASSOCIATED:
                        sgz_name = join(smpdir, smp_name + '.sgz')
                        with open(sgz_name, 'rb') as sgz:
                            smp_data = decompress(sgz.read())
